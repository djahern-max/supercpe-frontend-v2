 ...
 */
    ...
    /**
     * Get CE Broker dashboard data (REQUIRES AUTHENTICATION)
     */
    async getCEBrokerDashboard(licenseNumber) {
        const response = await apiClient.get(`/api/ce-broker/dashboard/${licenseNumber}`);
        return response.data;
    },

    /**
     * Export CPE records in CE Broker format (REQUIRES AUTHENTICATION)
     */
    async exportCEBrokerData(licenseNumber, format = 'json', options = {}) {
        ...
        const response = await apiClient.get(
            `/api/ce-broker/export/${licenseNumber}?${params}`
        );
        return response.data;
    },

    /**
     * Update CE Broker fields for a specific record (REQUIRES AUTHENTICATION)
     */
    async updateCEBrokerRecord(recordId, updateData) {
        const response = await apiClient.post(
            `/api/ce-broker/update-record/${recordId}`,
            updateData
        );
        return response.data;
    },
    ...
    /**
     * Mark records as exported to CE Broker (REQUIRES AUTHENTICATION)
     */
    async markCEBrokerExported(licenseNumber, recordIds) {
        const response = await apiClient.post(
            `/api/ce-broker/mark-exported/${licenseNumber}`,
    ...
    /**
     * Get CE Broker field options and mappings
     */
    async getCEBrokerOptions() {
        // This could be a static endpoint or computed on frontend
        return {
            course_types: [
                { value: 'live', label: 'Live (involves live interaction with presenter/host)' },
                { value: 'anytime', label: 'Anytime (is not date, time or location specific)' }
            ],
            delivery_methods: [
